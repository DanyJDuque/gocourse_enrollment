...
func TestService_Update(t *testing.T) {
	...
	t.Run("should return an error", func(t *testing.T) {
		...
		assert.NotNil(t, err)
		assert.Equal(t, wantCounter, counter)
		assert.EqualError(t, want, err.Error()) // valida un error con un string sí pasamos directamente el err, nos devuleve un error por formato
	})
...
func TestService_Create(t *testing.T) {
	...
	t.Run("should return an error in user sdk", func(t *testing.T) {
		var want error = errors.New("user sdk error")
		var wantCounter int = 1
		var counter = 0
		userSdk := &userSdk.UserSDKMock{
			GetMock: func(id string) (*domain.User, error) {
				counter++
				return nil, errors.New("user sdk error")
			},
		}
	...
	t.Run("should return an error in course sdk", func(t *testing.T) {
		var want error = errors.New("course sdk error")
		...
		courseSdk := &courseSdk.CourseSDKMock{
			GetMock: func(id string) (*domain.Course, error) {
				counter++
				return nil, errors.New("course sdk error")
			},
		}
	...
	t.Run("should return an error in repository", func(t *testing.T) {
		var want error = errors.New("repository error")
		...
		service := enrollment.NewService(l, userSdk, courseSdk, repoSdk)
	...
	t.Run("should create an enrollment", func(t *testing.T) {
		...
		assert.Equal(t, wantStatus, string(enrollment.Status))
	})
}
